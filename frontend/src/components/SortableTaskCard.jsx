...
export default function SortableTaskCard({ task, currentUser, onEdit, onDelete, isOverlay = false }) {
  ...
  return (
    <div 
      ...
      {...(canDrag ? attributes : {})} 
      {...(canDrag ? listeners : {})}
      className={`bg-[#464153] p-4 rounded-2xl border transition-all 
        ${!canDrag ? 'border-transparent opacity-50' : 'border-white/10 shadow-lg hover:border-[#D2C9D8]/30'}`}
    >
      <div className="flex justify-between items-start mb-2">
        <h4 className="text-sm font-bold text-white flex items-center gap-2">
          {task.title}
          {!canDrag && <FaLock className="text-white/20" size={10} />}
        </h4>
      </div>
      
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center gap-1.5 text-[10px] font-bold text-[#A29EAB]">
          <FaClock size={10} />
        ...
        {isBoss && !isOverlay && (
          <div className="flex gap-2">
            <button onPointerDown={(e) => { e.stopPropagation(); onEdit(task); }} className="text-[#A29EAB] hover:text-white p-1">
              <FaEdit size={12} />
            </button>
            <button onPointerDown={(e) => { e.stopPropagation(); onDelete(task._id); }} className="text-[#A29EAB] hover:text-rose-400 p-1">
              <FaTrash size={12} />
            </button>
          </div>
        )}
