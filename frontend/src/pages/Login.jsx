...
export default function Login() {
  const { login, error, isAuthenticated } = useAuth(); // Destructure Redux state & actions
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if the action was successful (Redux Toolkit specific check)
        ...
        <div className="hidden lg:flex lg:w-1/2 p-10 flex-col justify-between">
          ...
          <div className="space-y-4">
            <div className="bg-[#464153] p-4 rounded-2xl flex items-center gap-4">
              <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-[#35313F] text-sm font-bold shadow-sm">1</div>
              <div>
                <p className="font-bold text-white text-sm">Organized Management</p>
                <p className="text-[#A29EAB] text-xs mt-0.5">Keep everything in one place</p>
              </div>
            </div>
            <div className="bg-[#464153] p-4 rounded-2xl flex items-center gap-4">
              <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-[#35313F] text-sm font-bold shadow-sm">2</div>
              <div>
                <p className="font-bold text-white text-sm">Clear Tracking</p>
                <p className="text-[#A29EAB] text-xs mt-0.5">Never miss deadlines</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-sm">
            ...
            <div className="mb-6 relative">
              <div className="flex justify-between items-center mb-1.5 px-3">
                <label className="text-xs font-bold text-[#35313F] block">Password</label>
