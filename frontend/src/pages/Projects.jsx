...
export default function Projects() {
  const { token, user } = useAuth(); // User now contains the 'role'
  ...
  // 🔒 RBAC Check: Only Owners and Managers can perform administrative actions
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-[#A29EAB] text-[10px] md:text-xs font-medium mt-0.5">
                      {filteredProjects.length} Projects under management
                    </p>
                  </div>

                  <div className="flex flex-wrap items-center gap-3">
                    <div className="relative flex-1 min-w-[180px] md:min-w-[280px]">
                      <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-[#A29EAB]" size={12} />
                      <input
                        type="text"
                    ...
                    <div className="relative">
                      <FaFilter className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#A29EAB]" size={10} />
                      <select
                        value={statusFilter}
                    ...
                    {/* 🔒 CREATE BUTTON: Whitelisted for Admin/Owners */}
                    {canManage && (
                      <button
                        onClick={() => {
            ...
            <div className="max-w-[1600px] mx-auto w-full px-5 md:px-8 py-6">
              {filteredProjects.length === 0 ? (
                <div className="py-24 text-center bg-[#464153]/20 rounded-[2.5rem] border border-dashed border-white/10">
                  ...
                  <p className="text-[#A29EAB] text-xs mt-1">Refine your search or create a new mandate.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-5">
                  {filteredProjects.map((project) => (
                    <div
                      key={project._id}
                      className="bg-[#464153] rounded-[1.8rem] p-6 border border-transparent hover:border-white/10 transition-all flex flex-col justify-between group shadow-lg"
                    >
                      <div>
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <span className={`text-[9px] font-bold px-2.5 py-1 rounded-full uppercase tracking-widest ${getStatusColor(project.status)}`}>
                            {project.status}
                          </span>
                        </div>
                        ...
                        <div className="text-xs font-semibold text-[#A29EAB] mb-4 flex items-center gap-2">
                          <div className="w-5 h-5 rounded-full bg-[#D2C9D8] flex items-center justify-center text-[#35313F] text-[9px] font-bold">
                            {project.client?.name ? project.client.name.charAt(0).toUpperCase() : "S"}
                          </div>
                        ...
                        {canManage && project.budget > 0 && (
                          <div className="flex items-center gap-2 text-emerald-400 text-[10px] font-bold mb-3">
                        ...
                        <p className="text-xs md:text-sm text-[#A29EAB] font-medium mb-6 line-clamp-2 leading-relaxed">
                          {project.description || "Mandate overview pending."}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex justify-between items-center pt-4 border-t border-[#5B5569]/50">
                          <div className="text-[10px] font-bold text-[#A29EAB]">
                            {project.deadline ? (
                              <>
                                Due: <span className="text-white">{new Date(project.deadline).toLocaleDateString()}</span>
                              </>
                            ) : "Open Deadline"}
                          </div>

                          <div className="flex items-center gap-4">
                            {/* 🔒 EDIT: Only for higher roles */}
                            {canManage && (
                              <button
                                onClick={() => {
                            ...
                            {/* 🔒 DELETE: Strictly Managers/Owners */}
                            {canManage && (
                              <button
